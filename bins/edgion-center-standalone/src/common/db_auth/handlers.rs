...
pub async fn unified_login_handler(
    State(state): State<UnifiedLoginState>,
    ...
    if let Some(store) = &state.store {
            ...
            .await
        {
            ...
            Ok(false) => {}
            Err(e) => {
                tracing::error!(component = "db_auth", error = %e, "database credential verification failed");
    ...
    if state.single_admin_enabled {
        ...
        match tokio::task::spawn_blocking(move || checker.verify_single_admin(&username, &password))
            .await
        {
            Ok(true) => {
