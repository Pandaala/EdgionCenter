...
#[derive(Parser, Debug)]
#[command(
    name = "edgion-center-standalone",
    version,
    about = "SQL-backed Edgion Federated Center"
)]
...
impl EdgionCenterCli {
    ...
    pub async fn run(&self) -> Result<()> {
        ...
        let config: CenterConfig = if content.is_empty() {
            ...
            serde_yaml::with::singleton_map_recursive::deserialize(de).map_err(|e| {
                anyhow::anyhow!("Center config parse error ({}): {}", self.config_file, e)
            })?
        };
        ...
        let db: Option<Arc<crate::store::Store>> = if config.database.enabled {
            match crate::store::Store::connect(&config.database).await {
                ...
                Err(e) => {
                    // Standalone owns durable SQL state, so a connection or
                    // migration failure must fail before listeners are bound.
                    return Err(anyhow::anyhow!(
                        "Failed to connect to metadata store (backend = {:?}): {}",
        ...
        let cloudflare_waf_admin = edgion_center_integration_cloudflare::compose_waf_admin(
            &config.cloudflare_waf,
            provider_account_store.clone(),
            mounted_credential_resolver.clone(),
        )
        .map_err(|error| anyhow::anyhow!("Invalid Cloudflare WAF config: {error}"))?;
        ...
        let audit_log: Option<Arc<edgion_center_adapter_sql::audit::SqlAuditLog>> =
            if config.audit.enabled {
                match db.clone() {
                    Some(store) => {
                        ...
                        Some(Arc::new(
                            edgion_center_adapter_sql::audit::SqlAuditLog::spawn(store, 1024),
                        ))
                    }
                    None => {
                        tracing::warn!(
        ...
        let audit_sink: Option<crate::common::audit::AuditSink> = audit_log
            .clone()
            .map(|log| log as Arc<dyn edgion_center_core::AuditWriter>);
        let audit_log_reads = config.audit.log_reads;

        let controller_directory: Option<Arc<dyn edgion_center_core::ControllerDirectory>> =
            db.clone().map(|store| {
                Arc::new(edgion_center_adapter_sql::SqlControllerDirectory::new(
                    store,
                )) as Arc<_>
            });
        let sql_admin = db
            .clone()
            .map(|store| Arc::new(edgion_center_adapter_sql::SqlAdmin::new(store)));
        ...
        let grpc_server = FederationGrpcServer::new(
            ...
            config.sync.clone(),
            sync_client.clone(),
            controller_directory.clone(),
            trust_domain.clone(),
        );
        let grpc_server = match audit_sink.clone() {
        ...
        let controller_evictor: Arc<dyn edgion_center_runtime::eviction::ControllerEviction> =
            Arc::new(
                edgion_center_runtime::eviction::OwnerAwareControllerEvictor::local(local_evictor),
            );
        ...
        let api_state = ApiState {
            ...
            user_admin: sql_admin
                .clone()
                .map(|admin| admin as Arc<dyn edgion_center_core::UserAdmin>),
            role_admin: sql_admin.map(|admin| admin as Arc<dyn edgion_center_core::RoleAdmin>),
            ...
            platform_ready: Arc::new(std::sync::atomic::AtomicBool::new(
                !config.database.enabled || db.is_some(),
            )),
        ...
        let http_handle = tokio::spawn(async move {
            ...
            let app = match crate::api::web::WebSource::resolve(web_dir.as_deref()) {
                Some(source) => {
                    let source: std::sync::Arc<crate::api::web::WebSource> =
                ...
                None => {
                    tracing::info!(
                        component = "center",
                        "dashboard UI hosting disabled (pure-API mode)"
                    );
                    app
                }
            };
            ...
            let app =
                match crate::common::api::ip_allowlist::build_admin_ip_matcher(&allow_admin_ips)? {
                    Some(m) => {
                        tracing::info!(
                            component = "center",
                    ...
                    None => {
                        tracing::info!(
                            component = "center",
                            "admin IP allowlist inactive (allow all)"
                        );
                        app
                    }
                };
                        ...
                        Some(m) => {
                            ...
                            let acceptor =
                                axum_server::tls_rustls::RustlsAcceptor::new(rustls_config)
                                    .acceptor(crate::common::api::ip_acceptor::TcpIpAcceptor::new(
                                        m,
                                    ));
...
fn resolved_password_login_capability(config: &CenterConfig, database_ready: bool) -> bool {
    let single_admin = config.local_auth.as_ref().is_some_and(|value| {
        value.enabled && !value.username.is_empty() && !value.password.is_empty()
    });
    single_admin || (config.db_auth.enabled && database_ready)
}

fn validate_startup_policy(config: &CenterConfig) -> anyhow::Result<()> {
...
async fn build_access_app(
    ...
    if any_password_login {
        ...
        auth_state.update_local(crate::common::local_auth::LocalAuthState::from_config(
            &validator_cfg,
        ));
    }

    // Authorization store: RBAC -> DbAuthz (validated store_present), else AllowAll.
    let authz: std::sync::Arc<dyn edgion_center_core::Authorizer> = if rbac {
        let store = store_for_db
            .clone()
            .expect("validate_access guarantees a store when rbac is on");
        std::sync::Arc::new(edgion_center_adapter_sql::SqlAuthorizer::new(store))
    } else {
        std::sync::Arc::new(edgion_center_core::AllowAllAuthorizer)
    };
    ...
    let app = if any_password_login {
        ...
        let business = crate::common::db_auth::add_unified_auth_routes(
            base_router,
            login_state,
            auth_state.clone(),
        );
...
async fn bootstrap_admin(store: &crate::store::Store) -> anyhow::Result<()> {
    ...
    let username = std::env::var("EDGION_ADMIN_USERNAME")
        .ok()
        .filter(|v| !v.is_empty());
    let password = std::env::var("EDGION_ADMIN_PASSWORD")
        .ok()
        .filter(|v| !v.is_empty());
...
mod tests {
    ...
    fn validate_access_password_login_requires_secret() {
        ...
        assert!(
            err.to_string().contains("signing secret"),
            "unexpected: {err}"
        );
    }
    ...
    async fn build_access_rbac_without_store_errors() {
        ...
        let err = res
            .expect_err("rbac without a store must error")
            .to_string();
    ...
    async fn build_access_allow_all_oidc_only_mounts_no_login() {
        ...
        assert_eq!(
            status, 404,
            "no password login => /login must not be mounted (got {status})"
        );
    }
    ...
    async fn build_access_db_auth_mounts_login() {
        ...
        assert_ne!(
            status, 404,
            "db_auth on => /login must be mounted (got {status})"
        );
        assert_eq!(
            status, 401,
            "bad creds against the mounted login => uniform 401 (got {status})"
        );
    }
    ...
    fn skip_tls_fail_closes() {
        ...
        assert_eq!(
            decide_transport(&sec, Some("edgion.io")),
            TransportDecision::FailClose
        );
    }
    ...
    fn mtls_without_trust_domain_fail_closes() {
        assert_eq!(
            decide_transport(&mtls_sec(), None),
            TransportDecision::FailClose
        );
        assert_eq!(
            decide_transport(&mtls_sec(), Some("")),
            TransportDecision::FailClose
        );
    }
    ...
    fn center_boot_with_no_auth_config_does_not_panic() {
        ...
        let state =
            UnifiedAuthState::from_configs(auth_config, local_auth_config, require_auth, "test")
    ...
    async fn center_no_auth_config_admin_routes_return_503() {
        ...
        let business = Router::new().route("/api/v1/controllers", get(|| async { "controllers" }));
        let authz: Arc<dyn edgion_center_core::Authorizer> =
            Arc::new(edgion_center_core::AllowAllAuthorizer);
    ...
    #[test]
    fn password_login_capability_tracks_the_provider_that_is_really_composed() {
        let mut config = CenterConfig {
            local_auth: Some(crate::common::local_auth::LocalAuthConfig {
                enabled: true,
                username: "admin".into(),
                password: "a-long-enough-password".into(),
                jwt_secret: "a-long-enough-session-secret".into(),
                ..Default::default()
            }),
            ..Default::default()
        };
        assert!(resolved_password_login_capability(&config, false));

        config.local_auth.as_mut().unwrap().enabled = false;
        assert!(!resolved_password_login_capability(&config, false));

        config.db_auth.enabled = true;
        assert!(!resolved_password_login_capability(&config, false));
        assert!(resolved_password_login_capability(&config, true));
    }

    #[test]
    fn dedicated_standalone_requires_sql() {
        let mut config = CenterConfig::default();
        config.database.enabled = false;
        assert!(validate_startup_policy(&config).is_err());
    }
    ...
    fn test_admin_tls_cookie_warning_combinations() {
        ...
        assert!(
            msg.contains("admin_tls is enabled"),
        ...
        assert!(
            msg.contains("admin_tls is not configured"),
            "unexpected message: {msg}"
        );
    ...
    async fn test_rustls_config_loads_self_signed_pem() {
        ...
        assert!(
            res.is_ok(),
            "valid self-signed PEM should load: {:?}",
            res.err()
        );
    }
    ...
    async fn test_rustls_config_rejects_missing_path() {
        ...
        assert!(
            res.is_err(),
            "missing cert/key files must return Err, not panic"
        );
    }
}
