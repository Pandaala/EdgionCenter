---
name: center-db
description: Moved to skills/01-architecture/06-center/03-persistence.md. This file is kept as a redirect stub.
---

> **This file has moved.**
> Canonical location: [`skills/01-architecture/06-center/03-persistence.md`](../01-architecture/06-center/03-persistence.md)
...
- No `libsqlite3.so` is needed at runtime; deploying to `Dockerfile.runtime` (debian:bookworm-slim) requires no changes
...
- `Dockerfile.runtime` (debian:bookworm-slim + libssl3): no need to install `libsqlite3`; SQLite is statically linked into the binary
