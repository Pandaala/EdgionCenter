...
const EdgionTlsList = () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: ({ namespace, name, resourceVersion }: { namespace: string; name: string; resourceVersion: string }) =>
      resourceApi.delete(mutationTarget, 'edgiontls', namespace, name, resourceVersion),
    onSuccess: () => {
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 16 }}>
        <Search data-testid={resourceActionTestId('edgiontls', 'search')} placeholder={t('ph.searchNameNs')} value={searchText} onChange={(e) => setSearchText(e.target.value)}
