...
const UDPRouteList = () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: ({ namespace, name, resourceVersion }: { namespace: string; name: string; resourceVersion: string }) =>
      resourceApi.delete(mutationTarget, 'udproute', namespace, name, resourceVersion),
    onSuccess: () => {
  ...
  const batchDeleteMutation = useMutation({
    mutationFn: (resources: Array<{ namespace: string; name: string; resourceVersion: string }>) =>
      resourceApi.batchDelete(mutationTarget, 'udproute', resources),
    onSuccess: () => {
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 16 }}>
        <Search data-testid={resourceActionTestId('udproute', 'search')} placeholder={t('ph.searchNameNs')} value={searchText} onChange={(e) => setSearchText(e.target.value)}
