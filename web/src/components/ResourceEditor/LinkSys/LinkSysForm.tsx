        ...
        {type === 'webhook' && (() => {
          ...
          return (
            <Card title={t('linksys.webhook')} size="small">
              ...
              <Space wrap><Form.Item label="Timeout template"><Input value={webhook.timeoutMsTemplate} disabled={readOnly} onChange={(e)=>updateConfig({timeoutMsTemplate:e.target.value||undefined} as Partial<WebhookConfig>)}/></Form.Item><Form.Item label="Max response bytes"><InputNumber min={1} value={webhook.maxResponseBytes} disabled={readOnly} onChange={(maxResponseBytes)=>updateConfig({maxResponseBytes:maxResponseBytes??undefined} as Partial<WebhookConfig>)}/></Form.Item><Form.Item label="Allow degradation"><Switch checked={webhook.allowDegradation} disabled={readOnly} onChange={(allowDegradation)=>updateConfig({allowDegradation} as Partial<WebhookConfig>)}/></Form.Item><Form.Item label="Status on error"><InputNumber min={200} max={599} value={webhook.statusOnError} disabled={readOnly} onChange={(statusOnError)=>updateConfig({statusOnError:statusOnError??undefined} as Partial<WebhookConfig>)}/></Form.Item><Form.Item label="Degradation template"><Input value={webhook.allowDegradationTemplate} disabled={readOnly} onChange={(e)=>updateConfig({allowDegradationTemplate:e.target.value||undefined} as Partial<WebhookConfig>)}/></Form.Item><Form.Item label="TLS enabled"><Switch checked={webhook.tls?.enabled} disabled={readOnly} onChange={(enabled)=>updateConfig({tls:{...webhook.tls,enabled}} as Partial<WebhookConfig>)}/></Form.Item><Form.Item label="TLS verify"><Switch checked={webhook.tls?.verify} disabled={readOnly} onChange={(verify)=>updateConfig({tls:{...webhook.tls,verify}} as Partial<WebhookConfig>)}/></Form.Item></Space>
