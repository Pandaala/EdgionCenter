...
use edgion_center_adapter_cloudflare::{
    CloudflareApi, CloudflareApiToken, CloudflareCursorKey, CloudflareDnsAdapter,
    CloudflareHttpApi, CloudflareZoneInventory,
};
...
use edgion_center_app::api::cloudflare_dns::{
    ...
    CloudflareRecordPageRequest, CloudflareRecordSetDto, CloudflareRecordSetKey, CloudflareZoneDto,
    CloudflareZonePageDto, CloudflareZonePageRequest, SharedCloudflareDnsAdminService,
};
use edgion_center_core::{
    CloudProvider, CloudResourceId, CoreError, CoreResult, CredentialRef, CredentialSource,
    DnsPageRequest, DnsProvider, DnsRecordSetKey, DnsRoutingIdentity, DnsZoneId, DnsZoneRef,
...
impl CloudflareDnsReadService {
    ...
    async fn load_account(
        ...
        validate_account(&account, &self.cursor_key_ref)?;
        Ok(account)
    }
    ...
    async fn prepare(
        ...
        let (token, token_revision, cursor_key, cursor_revision) =
            self.resolve_material(&account).await?;
        let api = self
            .api_factory
            .build(token)
            .map_err(|_| CloudflareDnsAdminError::Unavailable)?;
        let adapter = CloudflareDnsAdapter::new(
            ...
            &account.spec,
            api,
            CloudflareCursorKey::from_zeroizing(cursor_key)
                .map_err(|_| CloudflareDnsAdminError::Unavailable)?,
        )
        .map_err(|_| CloudflareDnsAdminError::Unavailable)?;
        Ok((
    ...
    async fn resolve_material(
        &self,
        account: &ProviderAccount,
    ) -> Result<(CloudflareApiToken, String, Zeroizing<[u8; 32]>, String), CloudflareDnsAdminError>
    {
        ...
        Ok((token, token_revision, cursor_key, cursor_revision))
    }
    ...
    async fn recheck(&self, authority: &RequestAuthority) -> Result<(), CloudflareDnsAdminError> {
        ...
        let (_, token_revision, _, cursor_revision) = self.resolve_material(&current).await?;
        if token_revision != authority.token_revision
            || cursor_revision != authority.cursor_revision
        {
            return Err(CloudflareDnsAdminError::Unavailable);
        }
...
fn validate_account(
    ...
    if credential_ref == cursor_key_ref {
        return Err(CloudflareDnsAdminError::InvalidRequest);
    }
    Ok(())
}
...
async fn authoritative_zone(
    ...
    account_id: &CloudResourceId,
    zone_id: &DnsZoneId,
) -> Result<DnsZoneRef, NormalizedProviderError> {
    adapter
...
impl CloudflareDnsAdminService for CloudflareDnsReadService {
    ...
    async fn list_record_sets(
        ...
        self.run(account_id, move |adapter| async move {
            adapter.validate_record_inventory_cursor(zone_id, &request)?;
            let zone = authoritative_zone(&adapter, account_id, zone_id).await?;
            let page = adapter.list_record_sets(&zone, &request).await?;
...
mod tests {
    ...
    fn config_is_strict_bounded_and_redacted() {
        ...
        assert!(!format!("{config:?}").contains("cloudflare/cursor"));
