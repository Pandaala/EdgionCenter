...
//!
//! Uses Center's platform-neutral CIDR matcher so standalone builds do not pull
//! Kubernetes resource dependencies into the process.
...
use super::ip_matcher::{to_cidr_format, validate_ip_or_cidr, IpRadixMatcher};
...
pub fn validate_admin_ips(cidrs: &[String]) -> anyhow::Result<()> {
    for cidr in cidrs {
        validate_ip_or_cidr(cidr)
            .map_err(|e| anyhow::anyhow!("invalid allow_admin_ips entry: {}", e))?;
    }
    Ok(())
}
...
mod tests {
    ...
    fn app(m: Arc<IpRadixMatcher>) -> Router {
        Router::new().route("/foo", get(|| async { "ok" })).layer(
            axum::middleware::from_fn_with_state(m, ip_allowlist_middleware),
        )
    }
    ...
    async fn ipv4_mapped_ipv6_matches_ipv4_cidr() {
        let resp = run(
            app(matcher(&["10.0.0.0/8"])),
            Some("[::ffff:10.1.2.3]:5000"),
        )
        .await;
        assert_eq!(resp.status(), StatusCode::OK);
    }
