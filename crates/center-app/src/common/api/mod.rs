...
/// Add local password login/logout routes to the router using the shared
...
pub(crate) fn add_local_auth_routes(
    ...
    #[cfg(not(feature = "password-auth"))]
    {
        ...
        tracing::error!(
            component = "center",
            "local authentication requested but password-auth was not compiled"
        );
        router
    }

    #[cfg(feature = "password-auth")]
    use crate::common::local_auth::{login_handler, logout_handler};
    #[cfg(feature = "password-auth")]
    use axum::routing::post;

    #[cfg(feature = "password-auth")]
    let auth_router: Router<std::sync::Arc<crate::common::unified_auth::UnifiedAuthState>> =
        Router::new()
