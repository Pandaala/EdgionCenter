...
use super::{
    add_auth_identity_route, add_auth_status_route, add_local_auth_routes,
    cache_control_no_store_middleware, wrap_with_unified_auth_layer,
};
use crate::common::unified_auth::UnifiedAuthState;
use edgion_center_core::Authorizer;
...
/// 2. `/api/v1/auth/{login, logout}` (password handlers, mounted only when
///    `local_auth_intent = true`)
/// 3. `/api/v1/auth/{me,status}` (provider-neutral, mounted unconditionally)
...
///   `true`, mounts `/auth/{login,logout}`; when `false`, omits them
...
mod provider_neutral_tests {
    ...
    #[tokio::test]
    async fn oidc_only_composition_always_mounts_authenticated_identity() {
        let oidc = crate::common::auth::AdminAuthConfig {
            enabled: true,
            discovery: "https://identity.example.test/.well-known/openid-configuration".into(),
            audiences: vec!["edgion-center".into()],
            ..Default::default()
        };
        let state = UnifiedAuthState::from_configs(Some(&oidc), None, true, "test").unwrap();
        let app = compose_admin_routes(
            Router::new(),
            state,
            false,
...
#[cfg(all(test, feature = "password-auth"))]
mod tests {
    use super::*;
    use axum::{
        body::Body,
    ...
    use edgion_center_core::AllowAllAuthorizer;

    fn allow_all() -> std::sync::Arc<dyn Authorizer> {
    ...
    async fn db_authz_enforces_per_route_permissions() {
        ...
        let authz: Arc<dyn Authorizer> =
            Arc::new(edgion_center_adapter_sql::SqlAuthorizer::new(store));
    ...
    async fn oidc_rbac_unmapped_sub_403() {
        ...
        let authz: Arc<dyn Authorizer> =
            Arc::new(edgion_center_adapter_sql::SqlAuthorizer::new(store.clone()));
