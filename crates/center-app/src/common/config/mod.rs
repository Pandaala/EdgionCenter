...
fn resolve_from_work_dir(path: &str) -> PathBuf {
    let path = PathBuf::from(path);
    if path.is_absolute() {
        return path;
    }
    std::env::var_os("EDGION_WORK_DIR")
        .map(PathBuf::from)
        ...
        if !self.certs.is_empty() {
            if let Some(active) = &self.active {
                return self
                    .certs
                    .iter()
                    .find(|c| c.name.as_deref() == Some(active.as_str()));
            }
            if self.certs.len() == 1 {
        ...
        if let Some(active) = &self.active {
            if !self.certs.is_empty() {
                let found = self
                    .certs
                    .iter()
                    .any(|c| c.name.as_deref() == Some(active.as_str()));
                if !found {
                    anyhow::bail!(
                        "conf_sync_security: active='{}' does not match any cert name",
                        active
                    );
                }
            }
