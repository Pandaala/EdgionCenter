...
pub struct UnifiedAuthClaims {
    ...
    /// Validated OIDC group claim. Local tokens carry an empty list.
...
impl UnifiedAuthState {
    ...
    pub fn from_configs(
        ...
        for p in crate::common::auth::public_paths::PUBLIC_DISCOVERY_ENDPOINTS {
            skip_paths.insert((*p).to_string());
        }
        ...
        let oidc = match auth_config {
            Some(cfg) if cfg.enabled => {
                ...
                Some(AuthMiddlewareState::from_config(cfg).map_err(|e| {
                    anyhow::anyhow!("invalid [auth] config (component={}): {}", component, e)
                })?)
            }
            Some(_) => {
                tracing::info!(
                    component = component,
                    "OIDC authentication disabled by config"
                );
                None
            }
            None => {
        ...
        let local_initial = match local_auth_config {
            ...
            Some(_) => {
                tracing::info!(
                    component = component,
                    "Local authentication disabled by config"
                );
                None
            }
            None => {
        ...
        if local_auth_config.is_some_and(|config| config.enabled) {
            return Err(anyhow::anyhow!(
                "local authentication requires the password-auth build feature"
            ));
        }
...
pub async fn unified_auth_middleware(
    ...
    #[cfg(not(feature = "password-auth"))]
    let local_snapshot: Option<()> = None;
    ...
    let token = match extract_bearer_token(&request) {
        ...
        None => match extract_cookie_token(&request) {
            Some(t) => t,
            None => {
                return build_admin_auth_error_response(
    ...
    #[cfg(not(feature = "password-auth"))]
    let local_error: Option<String> = None;
    #[cfg(feature = "password-auth")]
...
async fn try_oidc_validation(
    state: &AuthMiddlewareState,
    token: &str,
) -> Result<UnifiedAuthClaims, String> {
    let claims = state.provider.validate(token).await?;
    let subject = validated_identity_claim("sub", claims.sub.as_deref(), 255)?;
    let issuer = validated_identity_claim("iss", claims.iss.as_deref(), 2_048)?;
    let groups = extract_validated_groups(&claims.claims, &state.groups_claim)?;
...
fn validated_identity_claim(
    name: &str,
    value: Option<&str>,
    max_bytes: usize,
) -> Result<String, String> {
    let value = value.ok_or_else(|| format!("OIDC token is missing {name:?} claim"))?;
    if value.is_empty() || value.len() > max_bytes || value.chars().any(char::is_control) {
        return Err(format!("OIDC token contains invalid {name:?} claim"));
    }
    Ok(value.to_string())
}
...
fn extract_validated_groups(
    ...
    let values = value
        .as_array()
        .ok_or_else(|| format!("OIDC group claim {claim_name:?} must be an array of strings"))?;
    if values.len() > 256 {
        return Err(format!(
            "OIDC group claim {claim_name:?} exceeds 256 entries"
        ));
    }
    ...
    for value in values {
        let group = value
            .as_str()
            .ok_or_else(|| format!("OIDC group claim {claim_name:?} must contain only strings"))?;
        if group.is_empty() || group.len() > 256 || group.chars().any(char::is_control) {
            return Err(format!(
                "OIDC group claim {claim_name:?} contains an invalid group"
            ));
        }
...
mod tests {
    ...
    #[test]
    fn oidc_groups_are_bounded_deduplicated_and_type_checked() {
        assert_eq!(
            extract_validated_groups(
                &serde_json::json!({"groups": ["platform-admins", "developers", "platform-admins"]}),
                "groups"
            )
            .unwrap(),
        ...
        assert!(
            extract_validated_groups(&serde_json::json!({"groups": "admins"}), "groups").is_err()
        );
        assert!(
            extract_validated_groups(&serde_json::json!({"groups": ["ok", 7]}), "groups").is_err()
        );
        assert!(
            extract_validated_groups(&serde_json::json!({"groups": ["bad\nname"]}), "groups")
                .is_err()
        );
    }

    #[test]
    fn oidc_subject_and_issuer_are_required_and_bounded() {
        assert!(validated_identity_claim("sub", None, 255).is_err());
        assert!(validated_identity_claim("sub", Some(""), 255).is_err());
        assert!(validated_identity_claim("sub", Some("bad\nsubject"), 255).is_err());
        assert!(validated_identity_claim("sub", Some(&"x".repeat(256)), 255).is_err());
        assert_eq!(
            validated_identity_claim("sub", Some("alice"), 255).unwrap(),
            "alice"
        );
        assert!(validated_identity_claim("iss", Some(&"x".repeat(2_049)), 2_048).is_err());
    }
    ...
    #[tokio::test]
    async fn server_info_is_public_for_pre_login_capability_discovery() {
        let state = UnifiedAuthState::from_configs(None, None, true, "test").unwrap();
        let app = axum::Router::new()
            .route(
                "/api/v1/server-info",
                axum::routing::get(|| async { "info" }),
            )
            .layer(axum::middleware::from_fn_with_state(
                state,
                unified_auth_middleware,
    ...
    async fn merge_before_layer_protects_all_routes() {
        let state = UnifiedAuthState::from_configs(None, None, true, "test").unwrap();
        let base: axum::Router =
            axum::Router::new().route("/base", axum::routing::get(|| async { "base-ok" }));
    ...
    fn local_validation_rejects_expired_token() {
        ...
        let err =
            try_local_validation(&local_state, &token).expect_err("expired token must be rejected");
    ...
    fn local_validation_rejects_pre_dated_nbf() {
        ...
        let err = try_local_validation(&local_state, &token)
    ...
    async fn configserver_paths_are_not_bypassed() {
        ...
        for path in [
            "/configserver/httproute/list",
            "/configserver/httproute?name=x",
        ] {
            let resp = app
                .clone()
                .oneshot(
    ...
    /// AuthBypass runs before provider-readiness checks, so no provider is needed.
    fn new_state_for_bypass() -> Arc<UnifiedAuthState> {
        UnifiedAuthState::from_configs(None, None, true, "test").unwrap()
    }
    ...
    async fn auth_bypass_marker_skips_authn() {
        let state = new_state_for_bypass();
        let app = axum::Router::new()
