...
use edgion_center_core::{AuditEvent, AuditFilter, Page};
...
impl From<AuditEvent> for AuditRecordDto {
    fn from(r: AuditEvent) -> Self {
        Self {
            ts: r.ts,
            actor: r.actor,
            provider: r.provider,
...
fn normalize(q: AuditQuery) -> (AuditFilter, Page) {
    ...
    (
        filter,
        Page::bounded(limit as u32, offset as u64, MAX_LIMIT as u32),
    )
}
...
pub async fn audit_list_handler(
    ...
    let (filter, page) = normalize(q);

    let Some(reader) = &state.audit_reader else {
    ...
    match reader.query(filter, page).await {
        Ok(result) => {
            let dtos: Vec<AuditRecordDto> =
                result.items.into_iter().map(AuditRecordDto::from).collect();
...
mod tests {
    ...
    async fn lists_with_filters() {
        ...
        let resp = audit_list_handler(State(state.clone()), Query(q))
            .await
            .into_response();
        ...
        let resp = audit_list_handler(State(state), Query(q))
            .await
            .into_response();
        let items = body_items(resp).await;
    ...
    async fn empty_when_db_disabled() {
        ...
        let resp = audit_list_handler(State(state), Query(AuditQuery::default()))
            .await
            .into_response();
        let items = body_items(resp).await;
