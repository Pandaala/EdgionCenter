...
mod tests {
    ...
    fn sanitize_allows_normal() {
        assert_eq!(
            sanitize_rel("assets/app.js").unwrap(),
            PathBuf::from("assets/app.js")
        );
        assert_eq!(
            sanitize_rel("./index.html").unwrap(),
    ...
    fn content_types() {
        ...
        assert_eq!(
            content_type_for("assets/style.css"),
    ...
    async fn fallback_is_public_while_api_stays_protected() {
        ...
        let authz: std::sync::Arc<dyn edgion_center_core::Authorizer> =
        ...
        assert_eq!(
            resp.status(),
            StatusCode::OK,
            "SPA route must serve the shell"
        );
