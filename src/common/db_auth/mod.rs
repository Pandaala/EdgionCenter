...
//! unchanged from `local_auth`. The installed `AuthzStore` (`AllowAllAuthz` vs
...
/// mount its own single-admin login/logout/me (the unified handler already owns
...
/// public-auth-endpoint skip list while `me` stays token-gated.
///
/// `auth_state` is required because the reused `logout_handler` reads
/// `cookie_secure` from the `UnifiedAuthState`'s local provider. `me_handler`
/// needs no state.
pub fn add_unified_auth_routes(
    ...
    // `login` needs UnifiedLoginState; `logout` needs the UnifiedAuthState; `me`
    // needs no state. Build each as its own fully-stated `Router<()>` then merge.
    ...
    business.merge(login).merge(logout).merge(me)
}
