...
use base64::Engine;
use openidconnect::core::{CoreJsonWebKey, CoreJsonWebKeySet, CoreJwsSigningAlgorithm, CoreProviderMetadata};
...
impl OidcProvider {
    ...
    pub fn from_config(config: &AdminAuthConfig) -> Result<Arc<Self>, String> {
        ...
        let issuer_url = IssuerUrl::new(issuer).map_err(|e| format!("Invalid OIDC issuer URL: {e}"))?;
    ...
    async fn fetch_and_cache(&self) -> Result<(), String> {
        ...
        let metadata = CoreProviderMetadata::discover_async(self.issuer_url.clone(), &self.http_client)
            .await
    ...
    async fn keys(&self, force: bool) -> Result<(CoreJsonWebKeySet, String), String> {
        ...
        let cached = guard.as_ref().ok_or_else(|| "OIDC JWKS unavailable".to_string())?;
    ...
    pub async fn validate(&self, token: &str) -> Result<OidcClaims, String> {
        ...
        let header_bytes = b64.decode(parts[0]).map_err(|e| format!("Invalid token header: {e}"))?;
        ...
        let alg: CoreJwsSigningAlgorithm =
            serde_json::from_value(Value::String(header.alg.clone())).map_err(|_| format!("unsupported alg: {}", header.alg))?;
        ...
        let signature = b64.decode(parts[2]).map_err(|e| format!("Invalid signature encoding: {e}"))?;

        let verified = verify_with_jwks(&jwks, &alg, header.kid.as_deref(), signing_input.as_bytes(), &signature);
        let discovery_issuer = match verified {
            ...
            Err(VerifyError::KidNotFound) => {
                ...
                verify_with_jwks(&jwks2, &alg, header.kid.as_deref(), signing_input.as_bytes(), &signature)
                    .map_err(|e| e.message())?;
                issuer2
            }
        ...
        let payload_bytes = b64.decode(parts[1]).map_err(|e| format!("Invalid token payload: {e}"))?;
        let claims: Value = serde_json::from_slice(&payload_bytes).map_err(|e| format!("Invalid token payload: {e}"))?;

        self.validate_claims(&claims, &discovery_issuer)?;

        let sub = claims.get("sub").and_then(Value::as_str).map(str::to_string);
        let iss = claims.get("iss").and_then(Value::as_str).map(str::to_string);
    ...
    fn validate_claims(&self, claims: &Value, discovery_issuer: &str) -> Result<(), String> {
        ...
        if !self.audiences.is_empty() {
            ...
            let ok = token_auds.iter().any(|a| self.audiences.iter().any(|x| x == a));
            if !ok {
...
fn verify_with_jwks(
    ...
    Err(VerifyError::Message(
        last_err.unwrap_or_else(|| "signature verification failed".to_string()),
    ))
}
...
mod tests {
    ...
    fn derive_issuer_strips_well_known() {
        ...
        assert_eq!(derive_issuer("https://idp.example.com/"), "https://idp.example.com");
    }

    #[test]
    fn extract_audiences_handles_string_and_array() {
        assert_eq!(extract_audiences(&serde_json::json!({"aud": "a"})), vec!["a".to_string()]);
        assert_eq!(
...
-----END PRIVATE KEY-----\n";
    ...
    fn provider_for(issuer: &str, audiences: Vec<String>, issuers: Vec<String>) -> Arc<OidcProvider> {
    ...
    async fn expired_token_rejected() {
        ...
        let token = sign(&json!({"sub": "alice", "iss": issuer, "exp": now() - 3600}), TEST_KID);
    ...
    async fn tampered_signature_rejected() {
        ...
        assert!(err.contains("signature") || err.contains("Unknown"), "got: {err}");
        drop(server);
    }
    ...
    async fn unknown_kid_rejected() {
        let (server, issuer) = mock_idp().await;
        let provider = provider_for(&issuer, vec![], vec![]);
        let token = sign(&json!({"sub": "alice", "iss": issuer, "exp": now() + 3600}), "nonexistent-kid");
        let err = provider.validate(&token).await.unwrap_err();
        assert!(err.contains("signing key") || err.contains("Unknown"), "got: {err}");
        drop(server);
    }
}
