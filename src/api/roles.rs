...
//! database is not configured (`state.db == None`) every mutating/listing
//! handler fails with 503; the catalog handler is static and always serves.
...
fn db_unavailable_list() -> axum::response::Response {
    (
        ...
        Json(ListResponse::<RoleDto>::error("database not configured".to_string())),
    )
        .into_response()
}
...
async fn to_dto(db: &crate::store::Store, role: Role) -> anyhow::Result<RoleDto> {
    let permission_keys = db.role_permissions(role.id).await?;
    Ok(RoleDto {
        id: role.id,
        name: role.name,
...
pub async fn list_roles_handler(State(state): State<ApiState>) -> impl IntoResponse {
    ...
    let roles = match db.list_roles().await {
        Ok(r) => r,
        Err(e) => {
            return (
                StatusCode::INTERNAL_SERVER_ERROR,
    ...
    let mut dtos = Vec::with_capacity(roles.len());
    for r in roles {
        match to_dto(db, r).await {
            Ok(d) => dtos.push(d),
            Err(e) => {
                return (
                    StatusCode::INTERNAL_SERVER_ERROR,
...
pub async fn create_role_handler(
    ...
    if req.name.trim().is_empty() {
        return (
            StatusCode::BAD_REQUEST,
            Json(ApiResponse::<String>::err_body("name must not be empty".to_string())),
        )
            .into_response();
    }
    ...
    if !keys.is_empty() {
        if let Err(e) = db.set_role_permissions(id, &keys).await {
...
pub async fn set_role_permissions_handler(
    ...
    match db.set_role_permissions(id, &req.permission_keys).await {
...
pub async fn delete_role_handler(State(state): State<ApiState>, Path(id): Path<i64>) -> impl IntoResponse {
    let Some(db) = &state.db else {
        return db_unavailable();
    };
...
mod tests {
    ...
    async fn roles_create_and_set_permissions() {
        ...
        let resp = list_roles_handler(State(state.clone())).await.into_response();
        ...
        assert_eq!(db.role_permissions(id).await.unwrap(), vec!["audit:read".to_string()]);
    }
    ...
    async fn permission_catalog_covers_all_keys() {
        ...
        let expected: BTreeSet<String> = catalog::all_keys().iter().map(|s| s.to_string()).collect();
        assert_eq!(seen, expected, "catalog endpoint must cover exactly all_keys()");
    }
