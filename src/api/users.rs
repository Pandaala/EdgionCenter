...
//!
//! Passwords are bcrypt-hashed (same crate + cost as `db_auth` login) before
//! they touch the store; the `password_hash` column is NEVER serialized back to
//! a client — [`UserDto`] deliberately omits it. When the database is not
//! configured (`state.db == None`) every handler fails with 503.
...
fn db_unavailable_list() -> axum::response::Response {
    (
        ...
        Json(ListResponse::<UserDto>::error("database not configured".to_string())),
    )
        .into_response()
}
...
fn db_unavailable() -> axum::response::Response {
    (
        StatusCode::SERVICE_UNAVAILABLE,
        Json(ApiResponse::<String>::err_body("database not configured".to_string())),
    )
        .into_response()
}
...
async fn to_dto(db: &crate::store::Store, user: User, role_names: &HashMap<i64, String>) -> anyhow::Result<UserDto> {
    let role_ids = db.user_role_ids(user.id).await?;
    let names = role_ids
        .iter()
...
/// Fetch a fresh `id -> role name` map from the store.
async fn role_name_map(db: &crate::store::Store) -> anyhow::Result<HashMap<i64, String>> {
...
pub async fn list_users_handler(State(state): State<ApiState>) -> impl IntoResponse {
    ...
    let role_names = match role_name_map(db).await {
        Ok(m) => m,
        Err(e) => {
            return (
                StatusCode::INTERNAL_SERVER_ERROR,
    ...
    for u in users {
        match to_dto(db, u, &role_names).await {
            Ok(d) => dtos.push(d),
            Err(e) => {
                return (
                    StatusCode::INTERNAL_SERVER_ERROR,
...
pub async fn create_user_handler(
    ...
    if req.username.trim().is_empty() {
        return (
            ...
            Json(ApiResponse::<String>::err_body("username must not be empty".to_string())),
        )
            .into_response();
    }
    if req.password.is_empty() {
        return (
            StatusCode::BAD_REQUEST,
            Json(ApiResponse::<String>::err_body("password must not be empty".to_string())),
        )
            .into_response();
    }

    // Reject a duplicate up-front with a clear 409 (the UNIQUE constraint would
    ...
    if let Some(role_ids) = req.role_ids {
        if let Err(e) = db.set_user_roles(id, &role_ids).await {
...
pub async fn update_user_handler(
    ...
    if let Some(password) = req.password {
        ...
        if let Err(e) = db.set_user_password(id, &hash).await {
            return internal_err(e);
        }
    }
...
pub async fn delete_user_handler(State(state): State<ApiState>, Path(id): Path<i64>) -> impl IntoResponse {
    let Some(db) = &state.db else {
        return db_unavailable();
    };
...
mod tests {
    ...
    async fn users_create_list_roundtrip() {
        ...
        let resp = create_user_handler(State(state.clone()), Json(req)).await.into_response();
    ...
    async fn users_duplicate_returns_409() {
        ...
        let resp = create_user_handler(State(state.clone()), Json(mk())).await.into_response();
        assert_eq!(resp.status(), StatusCode::CREATED);
        let resp = create_user_handler(State(state), Json(mk())).await.into_response();
    ...
    async fn users_patch_updates_status_password_roles() {
        ...
        // New password verifies via bcrypt; the old one no longer matches.
